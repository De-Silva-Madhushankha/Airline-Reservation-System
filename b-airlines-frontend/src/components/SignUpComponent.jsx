...
const onFinish = (values) => {
  ...
  .catch((error) => {
    if (error.response && error.response.status === 400) {
      if (error.response.data.message === 'Please enter a valid birthday') {
        message.error('Please enter a valid birthday');
      } else {
        message.error('User already exists. Enter a different email address.');
        navigate('/sign-up');
      }
    } else {
      console.log(error);
...
const validatePhoneNumber = (_, value) => {
  ...
  return (
    <div style={{ backgroundColor: 'white', padding: '50px 0', backgroundImage: 'light-pattern.jpg' }}>
      <div style={{ width: '100%', maxWidth: '600px', margin: '0 auto', backgroundColor: 'white', padding: '30px', borderRadius: '8px', boxshadow: '0 4px 8px rgba(0, 0, 0, 0.1)', border: '1px solid #ccc' }}>
